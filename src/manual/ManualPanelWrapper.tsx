...
const ManualPanelWrapper = forwardRef<
 ...
 const formatAddressForStorage = (address: string): string => {
    ...
    const deleteSelectedRegion = () => {
      if (
        manualPanelDrawingRef.current &&
        manualPanelDrawingRef.current.selectedRegionId !== null
      ) {
        ...
        setFullRegionData((prev) =>
          prev.filter((region) => region.id !== regionIdToDelete)
        );
        ...
        if (onRegionInfoChange) {
          ...
          const updatedRegions = fullRegionData.filter(
            (r) => r.id !== regionIdToDelete
          );
            ...
            if (manualPanelDrawingRef.current) {
              ...
              const processedRegions = updatedRegions.map((region) => {
                ...
                if (region.solarPanelData && region.solarPanelData.features) {
                  ...
                  region.selectedIds &&
                  Array.isArray(region.selectedIds)
                ) {
                  panelCount = region.selectedIds.filter(
      ...
      <ManualPanelDrawing
        ...
        initialRegionInfo={regions.map((r) => {
          ...
          if (r.selectedIds && Array.isArray(r.selectedIds)) {
            ...
            const selectedCount = r.selectedIds.filter(
              (item: any) => item.selected !== false
            ).length;
