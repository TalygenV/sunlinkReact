...
interface Region { id: number; panelCount: number; calculatedPanelCount?: number; solarPanelData?: any; features?: any[]; selectedIds?: any[]; }
...
export const ManualPanelControls: React.FC<ManualPanelControlsProps> = ({ totalManualPanels, onResetManualPanels, onDeleteRegion, onEnableDrawMode, onRotationChange,
  currentRotation, regions = [], selectedRegionId = null, onRegionSelect, obstructedPanelIds = new Set<string>(),
}) => {
  ...
  return (
    <div className="manual-panel-controls ">
      ...
      {regions.length > 0 && (
        <div className="mb-4">
          <div
            ...
            style={{ msOverflowStyle: "none", scrollbarWidth: "thin", cursor: "pointer", }}          >
            {regions.map((region, index) => (
              ...
              >
                ...
                {/* Button Core */}
                <div
                  className={`relative z-10 py-1 px-2 rounded-md text-xs text-center transition-all duration-500 ${selectedRegionId === region.id
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <motion.button variants={controlButtonVariants} whileHover="hover" whileTap="tap" onClick={() => {
          setTimeout(() => { if (onEnableDrawMode) { onEnableDrawMode(); } }, 200);
        }}
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="text-gray-400 text-sm">
            {selectedRegionId !== null
              ? `Section ${regions.findIndex((r) => r.id === selectedRegionId) + 1
              } Rotation`
              : "Section Rotation"}
          </div>
