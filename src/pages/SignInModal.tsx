import { createUserWithEmailAndPassword, getAuth, RecaptchaVerifier, sendPasswordResetEmail, signInWithEmailAndPassword, signInWithPhoneNumber, } from "firebase/auth";
...
export default function SignInModal({ isOpen, onClose, onSignInSuccess, }: SignInModalProps) {
    ...
    React.useEffect(() => {
        if (!isOpen) {
            ...
            if (step !== "phone") { trackEvent(AnalyticsEvents.FORM_ABANDON, { step }); }
    ...
    const setupRecaptcha = async () => {
        ...
        window.recaptchaVerifier = new RecaptchaVerifier(
            auth, recaptchaContainerRef.current,
            {
                size: "invisible",
                callback: () => {
                    // recaptcha solved
