...
interface UploadedFile {
  id: string; file: File; preview: string; category: string;
}
...
const PhotoUploadPage: React.FC<PhotoUploadPageProps> = ({ onComplete, customerInfo, }) => {
  ...
  const roofTypes = ["Asphalt Shingles", "Metal", "Tile (Clay/Concrete)", "Slate", "Wood Shingles", "Flat/Membrane", "Other",];
  const roofAges = ["0-5 years", "6-10 years", "11-15 years", "16-20 years", "21-25 years", "25+ years", "Unknown",];
  const uploadCategories = [{ id: "frontHome", title: "Front of Home", description: "Clear photo showing the front of your house and roof", icon: Home, required: true, },
  { id: "electricalPanel", title: "Electrical Panel", description: "Photo of your main electrical panel (breaker box)", icon: Zap, required: true, },
  { id: "meterClose", title: "Meter Close-up", description: "Close-up photo of your electric meter showing details", icon: Zap, required: true, },
  { id: "meterFar", title: "Meter Far Away", description: "Photo of your electric meter from a distance showing location", icon: Zap, required: true, },
  { id: "utilityBill", title: "Recent Utility Bill", description: "Photo of your most recent utility bill showing account holder name and account number", icon: FileText, required: true, },
  ];
  ...
  const handleFileSelect = (category: string, files: FileList | null) => {
    ...
    reader.onload = (e) => {
      const newFile: UploadedFile = { id: `${category}-${Date.now()}`, file, preview: e.target?.result as string, category, };
    ...
    return (
      <div className="bg-[#111] rounded-xl border border-gray-800 p-6">
        ...
        {existingFile ? (
          ...
          // Upload Area
          <div
            className={`border-2 border-dashed rounded-lg p-8 text-center transition-colors cursor-pointer ${isDraggedOver
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-4">
          <button
            ...
            className={`flex-1 py-4 px-8 rounded-xl font-semibold text-lg flex items-center justify-center space-x-3 transition-all duration-200 ${isFormComplete()
