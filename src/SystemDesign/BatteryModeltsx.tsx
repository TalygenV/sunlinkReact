...
const BatteryModeltsx = forwardRef<any, BatteryModelProps>(
  (
    ...
    const batteryMeshRef = useRef<Group | null>(null);
    const modelFile = useMemo(() => {
      return batteryId && modelUrls[batteryId] ? modelUrls[batteryId] : defaultModel;
    }, [batteryId]);
    ...
    useImperativeHandle(ref, () => ({
      rotateModel: (direction: "left" | "right" | "up" | "down") => {
    ...
    // Start auto rotation once initialized and model is loaded
    useEffect(() => {
      if (rendererReady && modelLoaded && mountRef.current?.sceneRef?.controls) {
        const controls = mountRef.current.sceneRef.controls;
        controls.autoRotate = false;
        controls.enabled = true;
        controls.enableRotate = true;
      }
    }, [rendererReady, modelLoaded]);
    ...
    useEffect(() => {
      ...
      controls.minPolarAngle = 0; // Limit vertical rotation (top)
      controls.maxPolarAngle = Math.PI; // Limit vertical rotation (bottom)
      ...
      loader.load(
        ...
        (gltf) => {
          ...
          batteryMeshRef.current = wrapper;
          ...
          const TARGET_SIZE = 2.5; // world units
      ...
      function updateModelProperties(currentPosition: string, currentScale: number, currentRotation: number) {
        ...
        if (currentPosition === "front") {
          Z_POSITION = 1; X_POSITION = isMobile ? -1.5 : 0; // Conditional X_POSITION
          Y_POSITION = -0.5;
        }
        else if (currentPosition === "middle") {
          MODEL_SCALE *= 0.7; Y_POSITION = 3; Z_POSITION = -1;
...
export default BatteryModeltsx;
